...
class HelpCog:
    ...
    async def helps(self, ctx):
        if ctx.invoked_subcommand is None:
            ...
            embed.add_field(name="Documentation",
                            value="<a:updating:403035325242540032> [https://scrapbox.io/GreenBOT](https://scrapbox.io/GreenBOT)")
            embed.add_field(name='__Note__',
                            value='Please be aware that next time GreenBOT updates, all the command below will be removed as all the help information will be avaliable on the website.')
            embed.add_field(name='Base',
                            value='All basic commands for the bot.')
            embed.add_field(name='Mindustry',
                            value='All Mindustry commands')
            guilds = []
            guilds = pickle.load(open("guilds.data", "rb"))
            for a in range(len(guilds)):
                if guilds[a][0] == ctx.guild.id:
            ...
            if ctx.guild.id == 462842304638484481:
                embed.add_field(name='Bounty',
                                value="All bounty commands for the Gear 5 server.")
    ...
    @helps.command(name='Mindustry',aliases=['mindustry','M','m'])
    async def Mindustry(self,ctx):
        embed = discord.Embed(title='Mindustry commands',
                              description="Type `&&help <category>` for more info on a category.\n[] Are optional.\n<> Are required.\n() Are aliases",
                              colour=0xFFAA00)
        embed.set_author(name=self.bot.user.name,
                        icon_url=self.bot.user.avatar_url_as(format='png'))
        ...
        embed.add_field(name='Aliases',
                        value='`Mindustry`,`mindustry`,`M`,`m`')
        embed.add_field(name='&&m',
                        value='Base Mindustry command.')
        embed.add_field(name='&&m ping <ip>',
                        value='Pings a server ip')
        embed.add_field(name='&&m servers',
                        value='Pings all the verified servers and displays a result. Can take a few minutes if a server is offline.')
        embed.add_field(name='&&m verify <ip>',
                        value='Submits an ip for verification.')
        embed.add_field(name='&&m invite UnOMSL',
                        value='Sends an invite link to the UnOfficial Mindustry Server List.')
        embed.add_field(name='&&m invite mindustry',
                        value="Explains how to get to the Offical Mindustry Discord.")
        embed.add_field(name='&&m game',
                        value="Displays the itch link to download the game.")
        embed.set_footer(text=ctx.guild.name,
                         icon_url=ctx.guild.icon_url_as(format='png'))

        await ctx.send(content='**Displaying base category help**', embed=embed)

    @helps.command(name='members',aliases=['Members','member','Member','Mem','mem'])
    @is_module_enabled('members')
    async def Hmembers(self,ctx):
        embed = discord.Embed(title='Members commands',
                              description="Type `&&help <category>` for more info on a category.\n[] Are optional.\n<> Are required.\n() Are aliases",
                              colour=0xFFAA00)
        embed.set_author(name=self.bot.user.name,
                        icon_url=self.bot.user.avatar_url_as(format='png'))
        ...
        embed.add_field(name='Aliases',
                        value='`Member`,`member`,`Members`,`members`,`Mem`,`mem`')
    ...
    async def Hmisc(self,cxt):
        ...
        embed.add_field(name='&&wanted [member]',
                        value='Sends a wanted poster with a random reward.')
        embed.set_footer(text=cxt.guild.name,
                         icon_url=cxt.guild.icon_url_as(format='png'))

        await ctx.send(content='**Displaying Misc category help**', embed=embed)

    @helps.command(name='Bounty',aliases=['bounty','Reward','reward','Worth','worth','Value','value'])
    async def HBounty(self,ctx):
        if ctx.guild.id == 462842304638484481:
            embed = discord.Embed(title='Bounty commands.',
                                  description="Type `&&help <category>` for more info on a category.\n[] Are optional.\n<> Are required.\n() Are aliases",
                                  colour=0x0099FF)
            embed.set_author(name=self.bot.user.name,
                            icon_url=self.bot.user.avatar_url_as(format='png'))
            ...
            embed.add_field(name='Aliases',
                            value='`Bounty`,`bounty`,`Reward`,`reward`,`Worth`,`worth`,`Value`,`value`')
            embed.add_field(name='&&bounty',
                            value='Base bounty command')
            embed.add_field(name='&&bounty check [member]',
                            value='Checks the bounty on a member. If nobody is mentioned then the author is selected.')
            embed.add_field(name='&&bounty rank [position]',
                            value='Checks the bounty on a member at the selected position. If no number is given then the most wanted user is shown.')
            embed.set_footer(text=ctx.guild.name,
                             icon_url=ctx.guild.icon_url_as(format='png'))

            await ctx.send(content='**Displaying Bounty category help**', embed=embed) 
